...
app.post(
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      return db.all('SELECT * FROM messages ORDER BY created_at DESC', [], (err, messages) => {
        res.render('contact', { lang, sent: false, error: true, messages: messages || [] });
      });
    }
...
app.post('/admin/upload', adminAuth, upload.array('images', 50), paintingValidators, async (req, res) => {
  ...
  // Upload is now handled browser-side via /admin/save-painting
...
app.post('/admin/delete/:id',
  ...
  (req, res) => {
    ...
    db.run('DELETE FROM paintings WHERE id = $1', [parseInt(req.params.id)], () => res.redirect('/admin/manage'));
  }
);
...
app.post('/admin/about',
  ...
  (req, res) => {
    ...
    db.run(
      'UPDATE about SET content_he=$1, content_en=$2, updated_at=CURRENT_TIMESTAMP WHERE id=1',
